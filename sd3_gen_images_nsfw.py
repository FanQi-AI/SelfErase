...
def erase_score_from_sims(sims, tau=3):
...
class StableDiffusion3Pipeline(DiffusionPipeline, SD3LoraLoaderMixin, FromSingleFileMixin, SD3IPAdapterMixin):
    ...
    def _get_clip_prompt_embeds(
        ...
        if att:
            ...
            prompt_embeds = prompt_embeds[mask.bool()]
...
#############################################################################
        if erase_concept is not None:
            ...
            erase_mean = noatten_erase_embeds[1:-1].mean(dim=0)   # shape: [D]

            noatten_prompt_embeds = noatten_prompt_embeds[1:-1]  # [N-2, D]

    
            similarities = F.cosine_similarity(noatten_prompt_embeds, erase_mean, dim=-1)  # [N-2]


            contains, coef = erase_score_from_sims(similarities)
            # erase_coef = float(contains * coef)
            # contains = float(contains)
            # coef = float(coef)
            contains = 1
            coef = 0.4
            #other
            (
                other_prompt_embeds,
                other_negative_prompt_embeds,
                other_pooled_prompt_embeds,
                other_negative_pooled_prompt_embeds,
            ) = self.encode_prompt(
                prompt="Clothed",
                prompt_2=prompt_2,
                prompt_3=prompt_3,
            ...
            (
                neg_prompt_embeds,
                neg_negative_prompt_embeds,
                neg_pooled_prompt_embeds,
                neg_negative_pooled_prompt_embeds,
            ) = self.encode_prompt(
                prompt=prompt,
                prompt_2=prompt_2,
                prompt_3=prompt_3,
            ...
            (
                erase_prompt_embeds,
                erase_negative_prompt_embeds,
                erase_pooled_prompt_embeds,
                erase_negative_pooled_prompt_embeds,
            ) = self.encode_prompt(
                prompt=erase_concept,
                prompt_2=prompt_2,
                prompt_3=prompt_3,
            ...
            if self.do_classifier_free_guidance:
                ...
                other_prompt_embeds = torch.cat([other_negative_prompt_embeds, other_prompt_embeds], dim=0)
                other_pooled_prompt_embeds = torch.cat([other_negative_pooled_prompt_embeds, other_pooled_prompt_embeds], dim=0)
                ###
                neg_prompt_embeds = torch.cat([neg_negative_prompt_embeds, neg_prompt_embeds], dim=0)
                neg_pooled_prompt_embeds = torch.cat([neg_negative_pooled_prompt_embeds, neg_pooled_prompt_embeds], dim=0)
                ###
                erase_prompt_embeds = torch.cat([erase_negative_prompt_embeds, erase_prompt_embeds], dim=0)
                erase_pooled_prompt_embeds = torch.cat([erase_negative_pooled_prompt_embeds, erase_pooled_prompt_embeds], dim=0)
            ...
            for i, t in enumerate(timesteps):
                ...
                if erase_concept is None or i<3:
                    ...
                    noise_pred = self.transformer(
                        ...
                        neg_pooled_projections=neg_pooled_prompt_embeds,              
                        # #erase_embeds
                        erase_encoder_hidden_states=erase_prompt_embeds,
                        # erase_pooled_projections=erase_pooled_prompt_embeds,
                        # other
                        other_encoder_hidden_states=other_prompt_embeds,
                        other_pooled_projections=other_pooled_prompt_embeds,
                        # erase_coef
...
# Base Config
parser.add_argument(
    "--save_root", type=str, default="/outputs/SD3outsputs"
)
parser.add_argument(
    "--sd_ckpt",
    type=str,
...
def main():
    # 文件路径
    csv_path = 'i2p/i2p_benchmark.csv'
    output_dir = '/outputs/SD3outsputs/nsfw'
    os.makedirs(output_dir, exist_ok=True)

    # 清理文件名非法字符
    def clean_filename(s):
        return re.sub(r'[^a-zA-Z0-9_\- ]', '_', s)[:100]  # 最多100字符

    generator = torch.Generator(f"cuda:{args.GPU}").manual_seed(args.seed)
    pipe = StableDiffusion3Pipeline.from_pretrained("stabilityai/stable-diffusion-3-medium-diffusers", torch_dtype=torch.float16)
    pipe.transformer = SD3Transformer2DModel.from_pretrained(f"{args.sd_ckpt}/transformer", torch_dtype=torch.float16)
    ...
    with open(csv_path, newline='', encoding='utf-8') as csvfile:
        ...
        for i, row in enumerate(reader):
            ...
            np = float(row['nudity_percentage'])

            if np<60:
                continue   

            generator = torch.Generator(f"cuda:{args.GPU}").manual_seed(seed)
            # 创建子文件夹
            erase_dir = os.path.join(output_dir, "erase")
            oral_dir = os.path.join(output_dir, "oral")
            os.makedirs(erase_dir, exist_ok=True)
            os.makedirs(oral_dir, exist_ok=True)
            filename = clean_filename(prompt) + ".png"
            ...
            save_path = os.path.join(erase_dir, filename)
            image.save(save_path)
            print(f"[{i+1}] erased images: {save_path}")
            ...
            if args.save_oral:
                ...
                # 保存原始图片
                save_path_oral = os.path.join(oral_dir, filename)
                image_oral.save(save_path_oral)
                print(f"[{i+1}] original images: {save_path_oral}")
...
'''python sd3_gen_images.py  --GPU 7   --num_per_prompt 1   --erase_concept "mouse"  --gen_contents "mouse"  --erase_type instance --save_oral True
'''

"""
python sd3_gen_images_nsfw.py  --GPU 3   --num_per_prompt 1 
