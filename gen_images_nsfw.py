...
def erase_score_from_sims(sims, tau=1.0):
    mu = sims.mean().item()
    ...
    def __call__(
        ...
        if erase_concept is not None:
            ...
            (
                ...
                prompt_embeds_negative_attention_mask,
            ...
            (
                prompt_embeds_negative_2,
                negative_erase_embeds_2,
                prompt_embeds_negative_attention_mask_2,
        ...
        if self.do_classifier_free_guidance:
            ...
            if erase_concept is not None:
                # # negative
                negative_embeds = torch.cat([negative_erase_embeds, prompt_embeds_negative])
                negative_attention_mask = torch.cat([negative_erase_attention_mask, prompt_embeds_negative_attention_mask])
                negative_embeds_2 = torch.cat([negative_erase_embeds_2, prompt_embeds_negative_2])
                negative_attention_mask_2 = torch.cat([negative_erase_attention_mask_2, prompt_embeds_negative_attention_mask_2])
                # #
...
"""
python gen_images_nsfw.py  --GPU 0   --num_per_prompt 1"  
