...
class gen_images(DiffusionPipeline):
    ...
    def encode_prompt(
        ...
        if prompt_embeds is None:
            ...
            if no_att is True:
                ...
                prompt_embeds = all_layers[0]       # 第5层的输出
            else:
    ...
    def __call__(
        ...
        if erase_concept is not None:
            # # sim no atten
            if "," in erase_concept:
                erase_list = erase_concept.split(",")  
            else:
                erase_list = [erase_concept]  
            ...
            erase_coef_list = []
            high_coef_indices = []
            for idx, erase_concept in enumerate(erase_list):
                ...
                if contains>0.4:
                    high_coef_indices.append(idx)
                erase_coef_list.append(erase_coef)
            erase_coef=max(erase_coef_list)

            selected_concepts = [erase_list[i] for i in high_coef_indices]

            erase_concept = " and ".join(selected_concepts)
...
# Base Config
parser.add_argument(
    "--save_root", type=str, default="/outputs/multi"
)
parser.add_argument(
    "--sd_ckpt",
    type=str,
...
"""
python gen_images_0.py  --prompt "a bad photo of a dog."  --GPU 0   --num_per_prompt 1   --erase_concept "Snoopy,apple tree"  
python gen_images_multi.py  --GPU 7   --num_per_prompt 1   --erase_concept "dog,cat,pig"  --gen_contents "dog,cat,pig"  --erase_type test --save_oral True
